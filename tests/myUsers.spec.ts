...
test.describe("User Management page verifications for RSPO Member", async () => {
  ...
  test('Verify the navigation to user management page', async ({ loginPage, dashboardPage, userManagementPage }) => {
  ...
  test('Verify the search bar using email,prisma_user_id,full_name', async ({ loginPage, dashboardPage, userManagementPage }) => {
    await loginPage.userlogin(credentials.userName, credentials.passWord)
    await dashboardPage.verifytheDashboardTitle("Dashboard")
    await dashboardPage.navigatestoEntityModule()
  ...
  test('Verify adding the existing user by email', async ({ loginPage, userManagementPage, dashboardPage }) => {
    await loginPage.userlogin(credentials.userName, credentials.passWord)
    await dashboardPage.verifytheDashboardTitle("Dashboard")
    await dashboardPage.navigatestoEntityModule()
  ...
  test('Verify adding the existing user by PrismaID', async ({ loginPage, userManagementPage, dashboardPage }) => {
    await loginPage.userlogin(credentials.userName, credentials.passWord);
    await dashboardPage.verifytheDashboardTitle("Dashboard");
    await dashboardPage.navigatestoEntityModule();
    await userManagementPage.verifytheAddExistingUserUsingEmail("Add existing user");
  ...
  test('Verify inviting a new user', async ({ loginPage, userManagementPage, dashboardPage }) => {
    await loginPage.userlogin(credentials.userName, credentials.passWord);
    await dashboardPage.verifytheDashboardTitle("Dashboard");
    await dashboardPage.navigatestoEntityModule();
  ...
  test('Verify removing added user', async ({ loginPage, userManagementPage, dashboardPage }) => {
    await loginPage.userlogin(credentials.userName, credentials.passWord);
    await dashboardPage.verifytheDashboardTitle("Dashboard");
    await dashboardPage.navigatestoEntityModule();
    await userManagementPage.verifyUserManagementTitle("User management");
